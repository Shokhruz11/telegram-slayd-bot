# -*- coding: utf-8 -*-
"""
SUPER TALABA BOT (GEMINI VERSIYA)
---------------------------------

Funksiyalar:
- Gemini (google-generativeai) orqali matn generatsiyasi
- Slayd (PPTX) generatsiyasi (fonli, 6 xil rangli dizayn)
- Mustaqil ish / Referat (DOCX)
- Kurs ishi (DOCX)
- Har bir foydalanuvchiga MAX_FREE_LIMIT marta bepul buyurtma
- Keyingi buyurtmalar 20 bet/slaydgacha pullik (PRICE_PER_ORDER)
- To'lov cheki rasm ko'rinishida yuboriladi, admin tasdiqlaydi
...
# Telegram bot
if not BOT_TOKEN:
    print("❌ BOT_TOKEN o'rnatilmagan! Railway Variables'ga qo'ying.")
...
if GEMINI_API_KEY:
    ...
    print("⚠️ GEMINI_API_KEY topilmadi, AI ishlamaydi.")
    gemini_model = None
...
def ask_ai(prompt: str) -> str:
    """
    Gemini orqali matn generatsiyasi.
    Xato bo'lsa, foydalanuvchiga yumshoq xabar qaytaradi.
    """
    if not gemini_model:
        return "ERROR: AI hozircha sozlanmagan. Admin GEMINI_API_KEY ni tekshirishi kerak."
...
def create_pptx_from_text(topic: str, ai_text: str, design_index: int = 1) -> str:
    ensure_files_dir()
    prs = Presentation()
...
def cmd_start(message):
    ...
    text = (
        "Assalomu alaykum!\n\n"
        "Bu bot orqali slayd, mustaqil ish / referat va kurs ishini tez va sifatli tayyorlab olishingiz mumkin.\n\n"
...
def cmd_help(message):
    text = (
        "Yordam bo'limi:\n\n"
        "- Menyudagi tugmalar orqali xizmat turini tanlang.\n"
        "- Slayd, referat/mustaqil ish yoki kurs ishiga buyurtma berishingiz mumkin.\n"
        f"- {MAX_FREE_LIMIT} ta bepul buyurtma huquqiga egasiz.\n"
        "- To'lovdan so'ng chek skrinshotini yuborasiz, admin tasdiqlaydi.\n\n"
...
if __name__ == "__main__":
    print("Super Talaba bot (Gemini) ishga tushmoqda...")
